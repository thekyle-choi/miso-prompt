...
| core_problem | "[사용자]는 [구조적·본질적 문제]를 겪고 있다" | "현장 직원은 표준화되지 않은 점검 방식으로 인해 데이터 신뢰성을 확보하지 못하고 있다" | "체크를 한다" (업무 서술일 뿐 문제 아님) |
| pain_points | "[구체적 고충 또는 비즈니스 손실]" | "엑셀 양식이 부서마다 달라서 매번 다시 작성해야 한다", "승인자가 누군지 몰라 메일을 여러 번 보낸다" | "수기로 한다" (방식 설명), "엑셀을 쓴다" (도구 설명), "복잡하다" (구체성 부족) |
